...
var (
	...
	// Max concurrent reconciles for all controllers created by this manager.
	defaultWorkers = 1
)
...
func init() {
	...
	rootCmd.PersistentFlags().Int("max-concurrent-reconciles", defaultWorkers, "Maximum number of concurrent reconciliations.")
...
func root() error {
	...
	maxConc := max(viper.GetInt("max-concurrent-reconciles"), 1)

	ctrlOpts := controller.Options{MaxConcurrentReconciles: maxConc}
